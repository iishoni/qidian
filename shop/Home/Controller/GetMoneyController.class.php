...
class GetMoneyController extends Controller {
    ...
    public function getAll(){
        ...
        if(!check_order_time()){
            ...
            $this->GetFreeze();
            $this->SevenDayClear();
            $this->push_top_parent();
            file_put_contents('./chaifen.txt', date('Y-m-d H:i:s')."执行完成\n",FILE_APPEND);
        }
    }
    ...
    public function SevenDayClear(){
        ...
        $time=7*60*60*24;
        $user=M('user');
        $info=$user->field('userid,username,account,unix_timestamp(now())-paidang_time as limit_time')->where($where)->having('limit_time > '.$time)->select();
        ...
        foreach ($info as $key => $value) {
            ...
            //清除冻结的金额
            $money_freeze->where("uid =".$userid." and isok = 0 and type = 'tj' and type = 'team'")->delete();
    ...
    //超时封号 只操作操作单个登录用户
    public function not_login($userid){
        ...
        $where['status']=1;
        $time=15*60*60*24;
        $user_list=$user->field('userid,username,account,unix_timestamp(now())-paidang_time as limit_time')->where($where)->having('limit_time > '.$time)->select();
